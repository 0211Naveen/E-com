...
import { userContext } from "../App"; // Import context to manage user session
    ...
    axios.post(`${process.env.REACT_APP_API_URL}/login`, { email, password })
        .then(result => {
            ...
            if (result.data.status === "Success") {
                ...
                const { userId, name } = result.data.user;
    ...
    return (
        ...
        <div className="d-flex justify-content-center align-items-center min-vh-100">
            <div className="bg-white p-4 rounded w-100" style={{ maxWidth: '400px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
                <h2 className="text-center form-title mb-4">Login</h2>
                ...
                <Link to="/register" className="btn btn-light border w-100 rounded-0 text-decoration-none">
                    Sign Up
                </Link>
            </div>
        </div>
